...
try {
    ...
    // Loop through each product and display it in the productBlock format
    foreach ($products as $product) {
        echo '<div class="productBlock">';
        echo '<div class="productImage"><img src="productImages/' . htmlspecialchars($product['product_image']) . '" alt="' . htmlspecialchars($product['product_name']) . '"></div>';
        echo '<p class="productName">' . htmlspecialchars($product['product_name']) . '</p>';
        echo '<p class="productDesc">' . nl2br(htmlspecialchars($product['product_description'])) . '</p>';
        echo '<p class="productPrice">$' . number_format($product['product_price'], 2) . '</p>';

        // Check if product_status is not empty and display it
        if (!empty($product['product_status'])) {
            echo '<p class="productStatus">' . htmlspecialchars($product['product_status']) . '</p>';
        }

        // Check inventory level and apply appropriate CSS class
        $inventoryClass = ($product['product_inStock'] < 10) ? 'productLowInventory' : '';
        echo '<p class="productInventory ' . $inventoryClass . '">#' . htmlspecialchars($product['product_inStock']) . ' In Stock</p>';
